...
(*
  ...
  Daqui retiramos algumas ideias, sendo que a maioria do algoritmo foi pensado por nós.

  As ideias retiradas foram:
  - Como podia ser feita a extrutura de uma NFA;
  - Para criar o autómato, aprendemos aqui como tratar uma união, concatenação e fecho;
  - Outros detalhes, como ter uma lista de estados atuais ao verificar a string com o autómato.
...
let rec printTransLst lst i =
  match lst with
    | a::b -> Printf.printf "\n %B S%d -%s> Sk " (getState a).isEnd i (getLetter a); printTransLst b i
    | [] -> ()
let rec printEpsilonTransLst lst i =
  match lst with
    | a::b -> Printf.printf "\n %B S%d -Ep> Sk " a.isEnd i; printEpsilonTransLst b i
    | [] -> ()
...
(*
  Criamos aqui o estado inicial. Este estado será o suficiente para representar todo o NFA.
*)
let initialState = createState false

(*
  Conjunto de funções onde criamos o autómato
  Para tratar a união, concatenação e fecho, recorremos à visualização dos diagramas destes, no link mencionado acima.
*)
...
(*
  ...
  Para evitar ciclos infinitos em trasições epsilon criamos uma lista visited.
*)
let visited = ref []
...
let rec getTransitions lst checkForChar return =
  match lst with
    ...
    | a::b -> if checkLetter a checkForChar
      then (if List.mem (getState a) !visited
        then getTransitions b checkForChar return
        else (visited := [(getState a)]@(!visited); getTransitions b checkForChar ([(getState a)]@return)))
...
let rec checkTransitions lst checkForChar return =
  match lst with
    | [] -> return
    | a::b -> if checkLetter a checkForChar
      then checkTransitions b checkForChar return@[(getState a)]
      else checkTransitions b checkForChar return

(*
  Verifica se existe um estado final entre os estados que está a analisar
  Se sim significa que pode terminar o programa com o output "YES"
*)
